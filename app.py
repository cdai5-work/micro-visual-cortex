...
def run_decoder_demo(direction, sharpness, completeness, brightness, noise, seed):
    ...
    prediction, spikes, metrics = decode_image(image, int(seed) + 100)
    conclusion = (
        "### Decoder解读结果\n"
        ...
        "Decoder只读取泊松脉冲重建出的空间活动和方向梯度，不读取输入标签。\n\n"
        "验证集准确率："
...
with gr.Blocks(title="微型视觉皮层 v0.2") as demo:
    gr.Markdown(
        "# 微型视觉皮层 v0.2\n"
        "从视觉刺激产生脉冲，并用可训练神经网络解读方向、边缘形态和轮廓完整性。"
    )
    with gr.Tabs():
        with gr.Tab("V1神经活动"):
            with gr.Row():
        ...
        with gr.Tab("基础视觉解码"):
            gr.Markdown(
                "## 让网络从脉冲中学习含义\n"
                "选择图形属性后，系统先生成神经脉冲，再由可更新的多任务MLP进行解读。"
            )
            with gr.Row():
                with gr.Column(scale=1):
