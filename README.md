...
触觉疼痛 → 触觉脉冲                         ├→ 可训练多任务 Decoder → 属性与危险判断
金属气味 → 嗅觉脉冲                         ┘
```

视觉 Decoder 不再直接读取原图、生成参数或正确标签，而是读取 64 个 V1 神经元的输出脉冲。触觉与嗅觉分别以 16 通道脉冲信号接入同一个通用 `MultimodalSignalBundle`。因此以后可以按同样接口添加听觉、温度、压力等模块。

> 这是教学性简化模型，不对应具体动物、脑区或临床结论。“危险”只是合成训练数据里的基础关联，不等于意识、真实语义理解或可靠安全判断。

## 当前能展示什么

- V1 页面：16×16 刺激、视网膜脉冲、四类方向选择性 V1 群体、膜电位和活动热图。
- 多模态解码页面：带方向的尖锐/圆滑、完整/缺口图形，以及疼痛和金属气味强度。
- Decoder 输出：上下左右、尖锐/圆滑、完整/缺口、有无疼痛、有无金属气味、危险/较安全。
- 示例关联：疼痛会提高危险标签；“向下 + 尖锐 + 金属气味”也被训练为危险线索。

Decoder 是一个纯 NumPy 多任务 MLP，共享隐藏层并具有六个输出头。首次进入解码页面时使用合成样本训练，之后在同一进程中复用。修改数据与训练参数后可重新训练，所以它是可更新网络，不是写死在界面里的答案。
...
prediction, bundle, retina, voltages, metrics, metadata = decode_multimodal(
    image, pain=0.8, metallic=0.9, seed=42
)
...
测试覆盖刺激范围、随机种子复现、亮度单调性、V1 方向选择性、模型复用、空间活动图、多模态信号尺寸、六个输出头及验证集准确率。
...
- 触觉与嗅觉目前是标量强度的群体脉冲编码，不是完整感觉器官模型。
